...
getUserInGroupsByGroupId(groupId:number): Observable<User[]>{
  ...
  addMessage(m:SentedMessege){
    return this._http.post<number>("/api/Portal",m);

  }
  // getAllMessages(groupId:number): Observable< SentedMessege[] >{
  //   return this._http.get<SentedMessege[]>("/api/User/"+groupId+"/Group");
  // }
